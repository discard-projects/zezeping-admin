<template>
  <div>
    ...
    <s-table :tableData="tableData">
      <template slot="attachment_images" slot-scope="{row, $index, intro}">
        <img v-for="attachmentImg in row.attachment_images" :src="attachmentImg.file.thumb.url" :key="attachmentImg.id" style="max-height: 30px">
      </template>
      <template slot="enabled" slot-scope="{row, $index, intro}">
        <ToggleSwitch v-model="row.enabled" :item="row" column="enabled" apiPath="/moments/:id/toggle_switch?field=enabled"></ToggleSwitch>
      </template>
...
export default {
  ...
  methods: {
    fetchData () {
      this._fetchData(this.api.getMoments(this.$route.query))
    }
  },
  mounted () {
